...
export class PharmacyRequestComponent implements OnInit, AfterViewInit {
  ...
  loadData2(pageIndex: number = 0, pageSize: number = this.pageSize) {
    ...
    this.http.get<any>(
      "http://localhost:8080/api/v1/admin/get-pharmacy-request", { params }
    ).subscribe((res) => {
  ...
  loadData() {
    this.http.get<PeriodicElement | PeriodicElement[]>(
      "http://localhost:8080/api/v1/admin/find-pharmacy-inactive?name=" + this.searchKey
    ).subscribe((res) => {
  ...
  active(id: any) {
    if (confirm("Are you sure to active this account ? " + id)) {
      this.http.get("http://localhost:8080/api/v1/admin/active-pharmacy?id=" + id)
        .subscribe(res => {

...
export class ReportUsersComponent implements OnInit, AfterViewInit {
  ...
  loadData2(pageIndex: number = 0, pageSize: number = this.pageSize) {
    ...
    this.http.get<any>(
      "http://localhost:8080/api/v1/admin/reported-users", { params }
    ).subscribe((res) => {
  ...
  loadData() {
    this.http.get<PeriodicElement | PeriodicElement[]>(
      "http://localhost:8080/api/v1/admin/find-reported-users?email=" + this.searchKey
    ).subscribe((res) => {
  ...
  active(id: any) {
    if (confirm("Are you sure to Re-Active this account ? " + id)) {
      this.http.get("http://localhost:8080/api/v1/admin/reactive-status-User?id=" + id)
        .subscribe(res => {
